...
| sign-out | a `POST` of `/sign-out` with a CSRF token from `authkit.CSRFIssue`, clearing the session and the recent list, then on to the issuer's RP-initiated logout with a `post_logout_redirect_uri` built from `ORIGOWEB_PUBLIC_URL` and never from the form. Written first as "clearing the cookie", which was this service's half alone: the single sign-on session outlived it and the next visit signed the person straight back in |
| front-channel logout | `GET /logout/notify`, which the issuer loads in a hidden frame when the person signs out elsewhere; it clears the same two cookies and answers 200. The one address here whose `frame-ancestors` is the issuer's origin instead of `'none'`, because a frame the browser refuses would leave the endpoint mounted and inert. It needs the client's `frontchannel_logout_uri` registered at the issuer, and an issuer on the same site as this installation, the session cookie being `SameSite=Lax` |
| CSRF | every `POST` route: sign-out, the mint form, the creation form, the visibility form, the deletion form, and the two on the key screen, each with a token from `authkit.CSRFIssue`. Everything else is a `GET` and changes nothing, except `GET /logout/notify`, which ends the session and carries no token because the issuer's hidden frame has none to send; the browser's `Sec-Fetch-Dest` header confines it to a frame, so a link cannot sign a person out. The route table in `internal/web/server.go` is the checked-in list `TestRoutesAreReadOnly` asserts. Written first as "the two `POST` routes", before the creation, visibility, deletion and mint screens were added to this spec |
